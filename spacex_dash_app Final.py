...
def get_scatter_chart(entered_site,payload_range):
    ...
    filt_spacex_df = spacex_df.loc[spacex_df['Payload Mass (kg)'] >= payload_range[0]]
    filt_spacex_df = filt_spacex_df.loc[filt_spacex_df['Payload Mass (kg)'] <= payload_range[1]]


    if entered_site == 'ALL':
        
        fig = px.scatter(filt_spacex_df, x ='Payload Mass (kg)', y = 'class',  
        title='Chart 1',
        color = 'Booster Version Category')

        return fig
    else:
        filtered_df = filt_spacex_df
        filtered_df = filtered_df.loc[filtered_df['Launch Site'] == entered_site]
